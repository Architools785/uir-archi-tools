...
export default function FreeShippingProgress({ compact = false }) {
  const { totalPrice, isFreeShipping, amountToFreeShipping, freeShippingProgress, isPickup } = useCart()

  return (
    <div style={{
      background: isFreeShipping || isPickup ? 'rgba(52,211,153,0.1)' : 'rgba(255,214,0,0.1)',
      border: `1px solid ${isFreeShipping || isPickup ? 'rgba(52,211,153,0.35)' : 'rgba(255,214,0,0.35)'}`,
      ...
      {isPickup ? (
        <p style={{
          ...
          ✅ Aucun frais, tu viens récupérer ta commande directement.
        </p>
      ) : isFreeShipping ? (
        <p style={{
          fontFamily: 'Outfit, sans-serif', fontWeight: 800,
